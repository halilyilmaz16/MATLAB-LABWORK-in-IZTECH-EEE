...
% This example shows how to automatically tune a PID Controller block using
% PID Tuner.
...
% can tune PID controller parameters to achieve a robust design with the
...
%% Open the Model
% Open the engine speed control model with PID Controller block and take a
% few moments to explore it.
open_system('scdspeedctrlpidblock')

%%
%% Design Overview
% In this example, you design a PI controller in an engine speed control
% loop. The goal of the design is to track the reference signal from a
% Simulink step block |scdspeedctrlpidblock/Speed Reference|. The design
% requirement are:
% 
...
% its block dialog box. In the *Main* tab, click *Tune*.
%
% <<../pid_blockdialog.png>>
...
%% 
% The following figure shows the PID Tuner dialog box with the initial design:
%
% <<../pid_initialtuner.png>>

%% Display PID Parameters
% Click *Show parameters* to view controller parameters P and I,
...
% Since we still have some room before reaching the settling time limit,
% you could reduce the overshoot by increasing the response time. Move the
% response time slider to the left to increase the closed loop response
...
% <<../pid_finaltuner.png>>
%
% <<../pid_finaltuner_params.png>>

%% Complete PID Design with Performance Tradeoff
% In order to achieve zero overshoot while reducing the settling time below
% 2 seconds, you need to take advantage of both sliders. You need to make
% control response faster to reduce the settling time and increase the
% robustness to reduce the overshoot. For example, you can reduce the
% response time from 3.4 to 1.5 seconds and increase robustness from 0.6 to
% 0.72.
%% 
% The following figure shows the closed-loop response with these settings:
%%
% <<../pid_finalextendedtuner.png>>
%
% <<../pid_finalextendedtuner_params.png>>
...
% *Update Block* in the PID Tuner. This action writes the parameters back 
% to the PID Controller block in the Simulink model.
%% 
% The following figure shows the updated PID Controller block dialog box:
%
% <<../pid_updatedblockdialog.png>>

%% Completed Design
% The following figure shows the response of the closed-loop system:
%%
% <<../pid_analysisresp.png>>
%
% The response shows that the new controller meets all the design
% requirements.     
%%
% You can also use the Control System Designer to design the PID
% Controller block, when the PID Controller block belongs to a multi-loop
% design task. See the example <docid:slcontrol_ug#bu6qrmj>.
